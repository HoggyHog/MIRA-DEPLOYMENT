...
var router3 = Router3();
router3.get("/content-generations", async (req, res) => {
  try {
...
router3.get("/content-generations/:id", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
...
router3.post("/save-exam", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "User not authenticated" });
...
router3.post("/save-lesson", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "User not authenticated" });
...
router3.post("/save-practice-analysis", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "User not authenticated" });
...
router3.patch("/content-generations/:id", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
    const contentId = parseInt(req.params.id);
    if (!userId) {
...
router3.delete("/content-generations/:id", async (req, res) => {
  try {
    const userId = req.auth?.user?.id || req.user?.userId;
    const contentId = parseInt(req.params.id);
    if (!userId) {
...
var attachUser = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const memoryUsers2 = global.memoryUsers || /* @__PURE__ */ new Map();
      const studentKey = `${req.auth.sub}_student`;
      const teacherKey = `${req.auth.sub}_teacher`;
      const studentUser = memoryUsers2.get(studentKey);
      const teacherUser = memoryUsers2.get(teacherKey);
      if (studentUser?.user) {
    ...
    if (user) {
      ...
      const mockUser = {
        ...
        name: "Demo User",
        role: "teacher",
        // Default to teacher for development
...
async function registerRoutes(app2) {
  ...
  app2.use("/api/teacher-content", verifyToken, attachUser, requireTeacher, teacherContentRoutes_default);
